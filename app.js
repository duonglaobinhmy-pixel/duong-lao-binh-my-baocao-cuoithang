...
//  - tab chi tiết: tìm kiếm, lọc, sort, và CHỈNH TAY (thêm/sửa/xóa) lưu trên máy
...
(function(){
  const ZCOLS=[
    ...
    ['tongXuat','Tổng xuất'],['tuVong','Tử vong'],['thanhLy','Thanh lý HĐ'],['dieuChuyen','Điều chuyển NB'],['diVien','Đi viện']
  ];
  const DCOLS=[['ct','Chỉ tiêu'],['zone','Khu'],['ma','Mã'],['ten','Họ tên','l'],['ngay','Ngày'],['cs','Cơ sở/Phòng','l'],['ck','Còn trong khu'],['gc','Ghi chú','l']];
  const EDITABLE=['ck','gc']; // cột cho sửa tay
  ...
  // gộp dữ liệu gốc + chỉnh tay
  function merged(){
    const e=loadEdits(), del=new Set(e.deleted);
    const rows=BASE.filter(r=>!del.has(rowKey(r))).map(r=>{
      const ov=e.edits[rowKey(r)]; return ov?{...r,...ov,_edited:true}:r;
    });
  ...
  // ---------- TAB 2: danh sách ----------
  let sortK='',dir=1,editMode=false;
  function renderDetail(){
    ...
    const body='<tbody>'+rows.map(r=>{
      ...
      const tds=DCOLS.map(c=>{
        if(c[0]==='ct')return '<td><span class="badge">'+esc(r.ct)+'</span></td>';
      ...
      return '<tr>'+tds.replace('</td>','</td>')+del+'</tr>'.replace('<tr>','<tr>'+ (tag?'':'') );
    }).join('')+'</tbody>';
  ...
  function addRow(){
    ...
    const r={ct:ct||'(thêm tay)',zone:prompt('Khu? (HH/TCDB1/...)')||'',ma:prompt('Mã?')||'',ten:prompt('Họ tên?')||'',
      ngay:prompt('Ngày? (yyyy-mm-dd)')||'',cs:prompt('Cơ sở/Phòng?')||'',ck:prompt('Còn trong khu? (Có/Không)')||'',gc:prompt('Ghi chú?')||''};
    const e=loadEdits(); e.added.push(r); saveEdits(e); renderDetail();
  }
  function exportJSON(){
    const out={month:MONTH,generatedAt:(new Date()).toLocaleString('vi-VN'),kpi:window.__KPI__||[],totals:totalsRef,detail:merged().map(({_edited,_added,...r})=>r)};
  ...
  function boot(d){
    ...
    document.getElementById('editToggle').onclick=()=>{editMode=!editMode;document.getElementById('editToggle').classList.toggle('on',editMode);document.getElementById('editTools').style.display=editMode?'flex':'none';renderDetail();};
